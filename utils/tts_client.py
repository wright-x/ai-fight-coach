...
class TTSClient:
    ...
    def synth(self, text: str, voice="Adam") -> 'AudioFileClip':
        """Return a MoviePy AudioFileClip generated by ElevenLabs for <text>."""
        try:
            if not self.api_key or not hasattr(self, 'elevenlabs'):
                print("⚠️ No ElevenLabs API key or import failed, skipping TTS")
                return None
            ...
            audio = self.elevenlabs.generate(
                text=text,
                voice="21m00Tcm4TlvDq8ikWAM",  # Use default voice ID
                model="eleven_monolingual_v1"
            )
            
            # Save to temp file
            self.elevenlabs.save(audio, temp_path)
            
            # Return MoviePy AudioFileClip
